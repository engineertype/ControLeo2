...
#define SECONDS_TO_MILLIS(x)    (((long)x)*1000L)
...
public:
  ...
  boolean handler() {
    ...
    if (counter % 40 == 0) {
      ...
      sprintf(debugBuffer, "bakeDuration = %u s = %ld ms", bakeDuration, SECONDS_TO_MILLIS(bakeDuration));
      Serial.println(debugBuffer);
    }
    */
...
protected:
  ...
  void serialDisplayBakingData() {
    ...
    Serial.println(debugBuffer);
    sprintf(debugBuffer, "Baking duration = %u seconds", this->bakeDuration);
    Serial.println(debugBuffer);
